    ...
    <script>
      (function () {
        ...
        try {
          ...
          var reverseDebounceTimer = null;
          var reverseIntervalTimer = null;
          var reversePending = null;
          var reverseInFlight = false;
          var reverseLastStartedAt = 0;
          var REVERSE_DEBOUNCE_MS = 450;
          var REVERSE_MIN_INTERVAL_MS = 350;
          ...
          function clearReverseTimers() {
            ...
            if (reverseIntervalTimer) {
              clearTimeout(reverseIntervalTimer);
              reverseIntervalTimer = null;
            }
          }

          function runReverseGeocode(job) {
            reverseInFlight = true;
            reverseLastStartedAt = Date.now();
            var finished = false;
            ...
            var timeout = setTimeout(function () {
              ...
              if (job.requestId === reverseRequestId) {
                emit({
                  type: 'selected',
                  location: {
                    address: fallbackAddress(job.point),
                    latitude: job.point.lat,
                    longitude: job.point.lng
                  }
                });
              }
              if (reversePending) flushReverseGeocode();
            }, 6000);

            geocoder.getLocation(job.point, function (result) {
              if (finished) return;
              finished = true;
              ...
              emit({
                type: 'selected',
                location: {
                  address: result && result.address ? result.address : fallbackAddress(job.point),
                  latitude: job.point.lat,
                  longitude: job.point.lng
                }
              });
          ...
          function flushReverseGeocode() {
            if (reverseInFlight || !reversePending) return;
            var elapsed = Date.now() - reverseLastStartedAt;
            var wait = reverseLastStartedAt === 0 ? 0 : Math.max(0, REVERSE_MIN_INTERVAL_MS - elapsed);
            if (wait > 0) {
              reverseIntervalTimer = setTimeout(function () {
                reverseIntervalTimer = null;
                flushReverseGeocode();
              }, wait);
              return;
            }
            var job = reversePending;
            reversePending = null;
            runReverseGeocode(job);
          }

          function scheduleReverseGeocode(point) {
            reverseRequestId += 1;
            reversePending = { point: point, requestId: reverseRequestId };

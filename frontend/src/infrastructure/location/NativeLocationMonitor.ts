...
function toWatchRequest(schedule: LocalReminderSchedule): LocationWatchRequest | null {
  if (schedule.schedule_type !== 'location' || schedule.status !== 'active') {
    return null;
  }
  const mode =
    schedule.reminder?.reminder_type === 'return_to_recorded_location' ? 'return' : 'arrive';
  const center =
    mode === 'return'
      ? schedule.runtime.recorded_location

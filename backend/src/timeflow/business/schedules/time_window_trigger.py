...
class TimeWindowTriggerService:
    ...
    def find_snapshots_entering_window(self, now: datetime) -> list[ScheduleSnapshot]:
        """返回在 `now` 时刻已进入提醒窗口的日程快照列表。"""
        return [
            schedule
    ...
    def find_schedules_entering_window(self, now: datetime) -> list[str]:
        ...
        return [schedule.schedule_id for schedule in self.find_snapshots_entering_window(now)]

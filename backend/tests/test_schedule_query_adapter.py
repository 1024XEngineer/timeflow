...
def test_adapter_carries_user_id_and_system_schedule_ref_id(session: Session) -> None:
    """The snapshot exposes `user_id` and `system_schedule_ref_id` for downstream dispatch."""
    start_time = datetime(2026, 7, 29, 15, 0)
    session.add(
        _make_schedule(
    ...
    snapshots = service.find_snapshots_entering_window(now=start_time)

    assert len(snapshots) == 1
    assert snapshots[0].user_id == "user_42"
    assert snapshots[0].system_schedule_ref_id == "system_schedule_42"

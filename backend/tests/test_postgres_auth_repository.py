...
def test_postgres_same_username_race_uses_one_winner_and_fresh_transaction(
    ...
    try:
        ...
        assert rows[0].password_hash not in caplog.text
...
def test_postgres_auth_uow_persists_only_committed_account(postgres_engine: Engine) -> None:
    ...
    try:
        ...
        assert [account.username for account in persisted] == [committed_username]
        assert persisted[0].password_hash.startswith("$argon2id$")
        assert hasher.verify("correct-password", persisted[0].password_hash)
    finally:
        with postgres_engine.begin() as connection:
            connection.execute(
